...
- When the Stream Dock host closes the plugin WebSocket, the plugin disposes its shared runtime, calls `VBVMR_Logout()`, and exits the plugin process so Voicemeeter does not retain stale Remote API sessions.
