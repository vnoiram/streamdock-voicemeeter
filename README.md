...
- If Voicemeeter is installed but not running, the plugin shows a clear "not running" error rather than guessing which edition to auto-launch. Once an edition has been detected in the current process lifetime, a later disconnect will attempt `VBVMR_RunVoicemeeter` with that same edition before retrying login.
...
- On plugin process startup, a new instance asks any already-running Voicemeeter plugin instance to shut down and waits for it to release the single-instance guard before connecting. This lets the old process call `VBVMR_Logout()` before the replacement process logs in.
- When the Stream Dock host closes the plugin WebSocket, the plugin stops shared-state polling, prevents any further `VBVMR_Login()` calls, calls `VBVMR_Logout()`, and exits the plugin process so Voicemeeter does not retain stale Remote API sessions.
...
```text
dist/release/streamdock-voicemeeter-0.1.17.zip
```
