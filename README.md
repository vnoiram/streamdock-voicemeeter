...
- When the Stream Dock host closes the plugin WebSocket or the plugin process exits normally, the plugin disposes its shared runtime and calls `VBVMR_Logout()` so Voicemeeter does not retain stale Remote API sessions.
...
```text
dist/release/streamdock-voicemeeter-0.1.0.zip
```
